...
class MochaArgumentBuilder
  ...
  #   testFilePattern: path pattern for finding test files
  ...
  build: (done) ->
    @_getTestFiles @config.testFilePattern, (err, files) =>
      if err then return done err
      done null, dargs(@config.mochaOptions).concat @config.testHelpers, @argv, files


  _getTestFiles: (testFilePattern, done) ->
    if minimist(@argv)._.length > 0
      done null, []
    else
      new TestFinder({@cwd, testFilePattern}).find (err, files) ->
        if err then return done err
