...
describe 'Mycha', ->
  ...
  describe 'run', ->
    ...
    beforeEach ->
      @mycha = new Mycha path.resolve('test_data', 'two_tests')
    ...
    context 'without arguments', ->
      ...
      it 'runs all the tests', ->
        expect(@mocha_argument).to.include_test_file 'two_tests', 'test', 'one_test.coffee'
        expect(@mocha_argument).to.include_test_file 'two_tests', 'test', 'two_test.coffee'
    ...
    context 'mycha run [filename]', ->
      ...
      beforeEach (done) ->
        @mycha.run
          run_options: {},
          run_files: [path.normalize('test/two_test.coffee')],
          done: =>
            @mocha_argument = @mocha_call_stub.args[0][0]
            done()
